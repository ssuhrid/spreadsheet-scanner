...
img=cv2.resize(img.copy(),(0,0),fx=0.3,fy=0.3)
orig=img.copy()

imSmall=cv2.resize(img.copy(),(0,0),fx=0.5,fy=0.5)
cv2.imshow('img',imSmall)
cv2.waitKey(0) 

gray=cv2.cvtColor(img,cv2.COLOR_BGR2GRAY);
imSmall=cv2.resize(gray.copy(),(0,0),fx=0.5,fy=0.5)
cv2.imshow('img',imSmall)
cv2.waitKey(0)


blur=cv2.GaussianBlur(gray,(5,5),0);
imSmall=cv2.resize(blur.copy(),(0,0),fx=0.5,fy=0.5)
cv2.imshow('img',imSmall)
cv2.waitKey(0) 

##blur=cv2.resize(blur.copy(),(0,0),fx=0.2,fy=0.2)
edges=cv2.Canny(blur,100,20)
imSmall=cv2.resize(edges.copy(),(0,0),fx=0.5,fy=0.5)
cv2.imshow('edges',imSmall)
cv2.waitKey(0) 
...
##    cv2.imshow("Outline", img)
    ...
    imSmall=cv2.resize(img.copy(),(0,0),fx=0.4,fy=0.4)
    cv2.imshow('img',imSmall)
    cv2.waitKey(0) 
...
##    cv2.imshow('warped',warped)
    imSmall=cv2.resize(warped.copy(),(0,0),fx=0.5,fy=0.5)
    cv2.imshow('warped',imSmall)
    cv2.waitKey(0) 
    ...
    imSmall=cv2.resize(thresh.copy(),(0,0),fx=0.5,fy=0.5)
    cv2.imshow('warped',imSmall)
    cv2.waitKey(0) 
...
##    thresh=cv2.resize(thresh.copy(),(0,0),fx=0.4,fy=0.4)
    ...
    text=image_to_string(Image.fromarray(np.array(orig,dtype='uint8')),cleanup=True)
    print "=====output=======\n"
    print text
cv2.destroyAllWindows()
