...
def showEachCell(im,imGrid):
    ...
    contours = arrangeCells(contours)
    for i in range(0, len(contours)):
        r = cv2.boundingRect(contours[i])
...
def arrangeCells(contours):
    ...
    for i in range(0,len(centroids)):
        for j in range(0,len(centroids)-1):
            if centroids[j][1]/K > centroids[j+1][1]/K :
               temp=centroids[j]
    ...
    for i in range(0,len(centroids)):
        for j in range(0,len(centroids)-1):
            if centroids[j][0]/K>centroids[j+1][0]/K and centroids[j][1]/K==centroids[j+1][1]/K :
               temp=centroids[j]
               centroids[j]=centroids[j+1]
               centroids[j+1]=temp
               temp=contours[j]
...
if __name__ == '__main__':
    ...
    img = cv2.imread('images//cam04.jpg');
    show('Image', img)
    cv2.waitKey(0);
...
####    cv2.rectangle(imGrid,(approx[0],approx[1]),(approx[0]+approx[2],approx[1]+approx[3]),127,thickness=10)
##    pts=approx.flatten()
##    pts=([pts[0],pts[1]],[pts[2],pts[3]],[pts[4],pts[5]],[pts[6],pts[7]])
##    
##    print pts
